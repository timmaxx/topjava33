...
Вверху вкладки _Network_ можно поставить фильтр по типу данных или url, а также есть полезные галочки _Preserve log_ для сохранения истории запросов после редиректа и _Disable cache_ для предотвращения кэширования (запросы GET могут кэшироваться браузером, также _Ctrl+F5_ позволяет перегрузить страницу целиком).
