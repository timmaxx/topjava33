...
`HttpStatus.NO_CONTENT` требуется только для `void` методов со статусом ответа HTTP.204  
...
> - Вынес создание `DataTable` в `topjava.common.js`. В параметр конфигурации добавляю общие опции используя [Spread syntax(...)](https://developer.mozilla.org/ru/docs/Web/JavaScript/Reference/Operators/Spread_syntax)
...
> - Выделил общую чать `dateOptions`, подключаю также через JS spread syntax(...)
...
- 5: Сделать в приложении выбор локали (см. http://javaops-demo.ru/topjava)
    - [Internationalization](https://terasolunaorg.github.io/guideline/5.4.1.RELEASE/en/ArchitectureInDetail/WebApplicationDetail/Internationalization.html )
